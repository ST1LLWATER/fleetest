...
module.exports = {
  ...
  getTrucks: async (req, res) => {
    try {
      ...
      const customoptiontrucks = await CustomOptionTruck.find({
        adminId: req.user.id,
      });

      // Render the EJS template and pass the trucks and customoptiontrucks as local variables
      res.render('trucksAdmin.ejs', {
        trucks: trucks,
        customoptiontrucks: customoptiontrucks,
        user: req.user,
      });
    } catch (err) {
      console.log(err);
  ...
  getEditTruck: async (req, res) => {
    try {
      ...
      const customOptions = await CustomOptionTruck.find({
        truckId: req.params.id,
      });
      res.render('editTruckAdmin.ejs', {
        truck: truck,
        user: req.user,
        driver: driver,
  ...
  getEditDriver: async (req, res) => {
    try {
      ...
      res.render('editDriverAdmin.ejs', {
        driver: driver,
        user: req.user,
        trucks: trucks,
  ...
  getSpreadsheets: async (req, res) => {
    try {
      ...
      const weekly = await WeeklySs.find({
        createdBy: { $in: driversIds },
        completed: { $eq: true },
      }).sort({ createdAt: 'desc' });
  ...
  filterSpreadsheets: async (req, res) => {
    try {
      ...
      const filteredSpreadsheets = await WeeklySs.find(filter).sort({
        createdAt: 'desc',
      });
      console.log(filter);
      res.render('spreadsheetsAdmin.ejs', {
        weekly: filteredSpreadsheets,
        user: req.user,
        trucks: trucks,
        drivers: drivers,
      });
  ...
  getIndividualSpreadsheet: async (req, res) => {
    try {
      ...
      res.render('individualSpreadsheetAdmin.ejs', {
        week: week,
        user: req.user,
